...
// ============================================================
// ENDPOINT EDIT LOKASI (HANYA ADMIN)
// ============================================================
    ...
    try {
        ...
        db.query(
            ...
            (err, rows) => {
                ...
                // ===== PILIHAN MODEL =====
                // UNCOMMENT SALAH SATU BARIS DI BAWAH INI:

                // OPSI 1: PAKAI MODEL ASLI (DEFAULT)
                let cmd = `"${pythonCmd}" "${pythonScript}" --json --data "${tempFile}"`;

                // OPSI 2: PAKAI MODEL MODIFIED (UNCOMMENT BARIS INI, COMMENT YANG DI ATAS)
                // let cmd = `"${pythonCmd}" "${pythonScript}" --model modified --json --data "${tempFile}"`;

                // OPSI 3: PAKAI MODEL BALANCED (UNCOMMENT BARIS INI, COMMENT YANG DI ATAS)
                // let cmd = `"${pythonCmd}" "${pythonScript}" --model balanced --json --data "${tempFile}"`;
...
// ============================================================
// ENDPOINT HISTORI PREDIKSI (DENGAN FILTER TANGGAL)
// ============================================================
app.get('/api/history', verifyToken, async (req, res) => {
    try {
        const { start_date, end_date, horizon, limit = 10, order = 'DESC' } = req.query;
        ...
        if (start_date && end_date) {
            ...
            if (dateRegex.test(start_date) && dateRegex.test(end_date)) {
                ...
                query += ' DATE(p.start_date) BETWEEN ? AND ?';
                params.push(start_date, end_date);
                hasWhere = true;
            } else {
                console.warn('⚠️ Format tanggal tidak valid:', { start_date, end_date });
            }
        } else if (start_date) {
            const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
            if (dateRegex.test(start_date)) {
                ...
                console.warn('⚠️ Format tanggal tidak valid:', start_date);
            }
        }
